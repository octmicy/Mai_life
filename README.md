...
- 配置私聊可注入关系角色、当前状态、未完话题、未回应摘要和生活记忆。
...
| 指令 | 用途 |
| --- | --- |
| `/麦麦`、`/麦麦帮助` | 返回冰蓝玻璃风格的本地指令菜单图，失败时退回纯文本 |
| `/麦麦状态` | 查看生活、消息管线、联网和后台任务状态 |
| `/麦麦日程` | 查看今日日程与当前细化场景 |
| `/麦麦关系` | 查看当前 QQ 用户的角色、温度和主动额度，需要启用用户档案 |
| `/麦麦撤回` | 查询本人私聊的短期撤回摘要，需要启用用户档案 |
| `/麦麦日记`、`/麦麦新闻`、`/麦麦探索` | 主人或管理员查看对应记录 |
| `/麦麦日期`、`/麦麦添加日期`、`/麦麦删除日期`、`/麦麦确认日期` | 管理当前用户的重要日期，需要启用用户档案 |
| `/麦麦书柜`、`/麦麦阅读 序号或完整编号` | 查看当前关系有权访问的书柜；阅读时输入 `/麦麦书柜` 列表里的序号或完整编号 |
| `/麦麦转述 群QQ号 内容` | 主人或管理员发起简单群转述 |
| `/麦麦统计` | 管理员查看模型 Token 与搜索 API 请求统计 |
| `/麦麦管理 [概览/用户/群聊/日期/来源/书柜/统计/主动/搜索]` | 管理员查看脱敏摘要（搜索为本地搜索历史） |
| `/麦麦配置` | 查看配置摘要 |
| `/麦麦重生日程`、`/麦麦休息测试`、`/麦麦立即创作` | 管理员诊断或手动任务 |

所有指令仅允许在私聊中使用。启用用户档案的 QQ 可使用个人功能；管理员 QQ 可直接使用菜单和管理类功能。管理员、主人和用户身份始终只按 QQ 号判断。
...
私密话题、撤回摘要、搜索 Key 状态和 Token 明细不会通过无鉴权公共 API 暴露。
