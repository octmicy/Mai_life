...
class MaiLifeMenuRenderer:
    ...
    def _find_font_paths(explicit:str="")->tuple[str,str]:
        ...
        bold_candidates=[
            root/"assets"/"font-bold.ttf",Path("C:/Windows/Fonts/NotoSansSC-VF.ttf"),
    ...
    def _draw_section(self,image:Any,box:tuple[int,int,int,int],section:CommandSection,
        ...
        draw.text((x1+34,y1+20),section.title,font=fonts["section"],fill=self._PALETTE["title"])
        y=y1+66
        ...
        for index,(item,values) in enumerate(zip(section.items,layout)):
            ...
            for line in command_lines:
                draw.text((x1+24,y),line,font=fonts["command"],fill=accent); y+=command_h
            y+=2
            for line in description_lines:
                draw.text((x1+24,y),line,font=fonts["description"],fill=self._PALETTE["body"]); y+=description_h
    ...
    def render(self,title:str,sections:Sequence[CommandSection],*,version:str="1.8.0",notice:str="")->bytes:
        ...
        try:
            fonts={
                "eyebrow":self._font(18,bold=True),"title":self._font(43,bold=True),
            ...
            draw.text((margin,132),"MAI LIFE  /  LOCAL COMMAND MENU",font=fonts["eyebrow"],fill=self._PALETTE["accent"])
            draw.text((margin,166),str(title or "麦麦生活 · 指令中心"),font=fonts["title"],fill=self._PALETTE["title"])
            draw.text((margin,226),"同一条生活时间线，安静记录每一天",font=fonts["subtitle"],fill=self._PALETTE["muted"])
            badge_width=120
            ...
            draw.text((self.WIDTH-margin-badge_width+22,154),f"v{version}",font=fonts["eyebrow"],fill=self._PALETTE["accent"])
            ...
            if clean_notice:
                ...
                draw.text((margin+16,273),clean_notice,font=fonts["footer"],fill="#6A7195")
